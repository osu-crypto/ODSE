...
int Client_ODSE::initMemory()
{
    ...
    aes_keys = new unsigned char[MATRIX_COL_SIZE];
    search_res = new MatrixType[MATRIX_COL_SIZE];
...
int Client_ODSE::sendEncryptedIndex()
{
    ...
    if(choice=='y')
    {
        ...
        printf("Sending Matrix I to server(s)...");
...
int Client_ODSE::searchKeyword(string keyword, TYPE_COUNTER &number)
{
    ...
    try
    {
        if(ready = false)
        {
            printf("Please load/build encrypted index first");
            number=-1;
            return -1;
        }
        printf("\n\n Searching \"%s\" ....\n\n", keyword.c_str());
        ...
        for(int s = 0 ; s <NUM_SERVERS;s++)
        {
            ...
            get_data_param[s].idx_vector = tau.row_vector[s];
        ...
        for(int i = 1 ; i < NUM_SERVERS;i++)
        {
            ...
            {
                search_data[0][j].byte_data ^= search_data[i][j].byte_data;
            } 
        }
        // IND-CPA decryption
        odse_keygen->enc_dec_preAESKey(search_res,search_data[0],aes_keys,MATRIX_COL_SIZE);
...
int Client_ODSE::sendSearch_query(int server_id, unsigned char* row_vector, MatrixType* I_prime) 
{
    ...
    {   
        ...
        socket.send(row_vector,MATRIX_ROW_SIZE/BYTE_SIZE);
