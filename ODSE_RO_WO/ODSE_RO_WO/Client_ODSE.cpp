...
int Client_ODSE::initMemory()
{
    ...
    I_prime = new MatrixType[WRITE_ORAM_LAMBDA*MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE/BYTE_SIZE];
    memset(I_prime,0,WRITE_ORAM_LAMBDA*MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
    ready = true;
    
    this->decrypt_col_key = new unsigned char[WRITE_ORAM_LAMBDA * MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE/BYTE_SIZE];
    this->reencrypt_col_key = new unsigned char[WRITE_ORAM_LAMBDA * MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE/BYTE_SIZE];
    memset(decrypt_col_key, 0, WRITE_ORAM_LAMBDA * MATRIX_ROW_SIZE * ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
    memset(reencrypt_col_key, 0, WRITE_ORAM_LAMBDA * MATRIX_ROW_SIZE * ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
    ...
    search_aes_keys = new unsigned char[MATRIX_COL_SIZE];
    memset(search_aes_keys,0,MATRIX_COL_SIZE);
...
int Client_ODSE::loadState()
{
    ...
    key_loc = gcsClientStatePath + "key2";
    ...
    Miscellaneous::read_array_from_file(FILENAME_BLOCK_COUNTER_ARRAY,gcsClientStatePath,this->block_counter_arr,NUM_BLOCKS);
    printf("OK!\n");
    ...
    //generate row keys for precomputation
    memset(this->row_keys,0,BLOCK_CIPHER_SIZE*MATRIX_ROW_SIZE);
    ODSE_KeyGen odse_keygen;
    odse_keygen.pregenerateRow_keys(row_keys,this->masterKey);
...
int Client_ODSE::saveState()
{
    ...
    Miscellaneous::write_array_to_file(FILENAME_BLOCK_COUNTER_ARRAY,gcsClientStatePath,this->block_counter_arr,NUM_BLOCKS);
    printf("OK!\n");
...
int Client_ODSE::createEncryptedIndex()
{
    ...
    try
    {
        ...
        printf("Setting up data structure......\n");
...
int Client_ODSE::searchKeyword(string keyword, TYPE_COUNTER &number)
{
    ...
    try
    {
        ...
        printf("Generating keyword token...");
                
        odse->searchToken(  tau, keyword, this->T_W, this->masterKey);
        printf("OK!\n");
        for(int s = 0 ; s < NUM_SERVERS; s++)
        {
            //serializiation
        ...
        THREAD_PRECOMPUTE_AESKEY aes_key_decrypt_param(search_aes_keys,tau.row_index,ROW,this->block_counter_arr,this->row_keys,this->masterKey);
        pthread_create(&thread_precomputeAesKey_decrypt,NULL,&Client_ODSE::thread_precomputeAesKey_func,(void*)&aes_key_decrypt_param);
        
        THREAD_GETDATA get_data_param[NUM_SERVERS];
        
        for(int s = 0 ; s < NUM_SERVERS;s++)
        {
            get_data_param[s].data = search_data[s];
        ...
        zz_p res[MATRIX_COL_SIZE/(FF_SIZE/BYTE_SIZE)];

        zz_p tmp;
        
        for(TYPE_INDEX col = 0 ; col < MATRIX_COL_SIZE/(FF_SIZE/BYTE_SIZE);col++)
        {
            zz_p shares[NUM_SERVERS];
            for(int i = 0 ; i < NUM_SERVERS ; i++)
            {
                shares[i] = search_result[i][col];
            }
            res[col] = odse->simpleRecover(shares);
        }
        
        
        //desiralize the res
        memset(search_data[0],0,MATRIX_COL_SIZE);
        odse->serializeFFElementVector((unsigned char*)search_data[0],res,MATRIX_COL_SIZE/(FF_SIZE/BYTE_SIZE));
        
        // IND-CPA decryption
        odse_keygen->enc_dec_preAESKey(mat_search_res,search_data[0],search_aes_keys,MATRIX_COL_SIZE);
        ...
        for(TYPE_INDEX ii=0; ii<MATRIX_COL_SIZE; ii++)
        {
                ...
                {
                    if(((bit_number+(ii*BYTE_SIZE)) % FF_SIZE) >= (NP_BITS-1))
                        continue;
                    if(BIT_CHECK(&mat_search_res[ii].byte_data,bit_number))
 ...
 * @return	0 if successful
 */
int Client_ODSE::sendSearch_query(int server_id, unsigned char* row_vector, MatrixType* I_prime) 
{
    int cmd;
 ...
 * @return	0 if successful
 */
int Client_ODSE::downloadColumnBlock(int server_id, std::set<TYPE_INDEX> indexes, MatrixType* I_prime) 
{
    int cmd;
    unsigned char buffer_in[SOCKET_BUFFER_SIZE] = {'\0'};
    unsigned char buffer_out[SOCKET_BUFFER_SIZE] = {'\0'};
    ...
    {   
        ...
        /* deserialize */
        memcpy(I_prime,serialized_buffer,WRITE_ORAM_LAMBDA*(MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE)/BYTE_SIZE);
    }
    catch(exception &ex)
    {
        ret = -1;
 ...
 * @return	0 if successful
 */
int Client_ODSE::uploadColumnBlock(int server_id, std::set<TYPE_INDEX> indexes, MatrixType *I_prime)
 {
    int cmd;
    Miscellaneous misc;
    unsigned char buffer_in[SOCKET_BUFFER_SIZE] = {'\0'};
...
int Client_ODSE::updateFile(string filename, string path)
{
    ...
    try
    {
        ...
        printf("Getting lambda columns/blocks from 1 server...\n");
        THREAD_GETDATA get_data_param(selectedIdx,this->I_prime);
        get_data_param.server_id = rand() % NUM_SERVERS;
        pthread_create(&thread_getData[0],NULL,&Client_ODSE::thread_downloadData_func,(void*)&get_data_param);
        pthread_join( thread_getData[0], NULL);
        
        memset(this->decrypt_col_key,0,WRITE_ORAM_LAMBDA *  MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
        memset(this->reencrypt_col_key,0,WRITE_ORAM_LAMBDA *  MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
        THREAD_PRECOMPUTE_AESKEY aes_key_decrypt_param(this->decrypt_col_key,selectedIdx,COL,false,this->block_counter_arr,this->row_keys,this->masterKey);
        pthread_create(&thread_precomputeAesKey_decrypt,NULL,&Client_ODSE::thread_precomputeAesKey_func,(void*)&aes_key_decrypt_param);
        
        THREAD_PRECOMPUTE_AESKEY aes_key_reencrypt_param(this->reencrypt_col_key,selectedIdx,COL,true,this->block_counter_arr,this->row_keys,this->masterKey);
        pthread_create(&thread_precomputeAesKey_reencrypt,NULL,&Client_ODSE::thread_precomputeAesKey_func,(void*)&aes_key_reencrypt_param);        
        pthread_join(thread_precomputeAesKey_decrypt, NULL);
        pthread_join(thread_precomputeAesKey_reencrypt, NULL);
       
        // increase counter
        for(std::set<TYPE_INDEX>::iterator i = selectedIdx.begin(); i != selectedIdx.end() ; ++i )
        {
            this->block_counter_arr[(*i)]++;
        }
        
        printf("Peforming update...\n");
        odse->updateToken(filename,path,selectedIdx,this->I_prime,this->S,this->T_F,this->T_W,this->lstDummy_column_idx, this->lstFree_row_idx,extracted_keywords,this->decrypt_col_key,this->reencrypt_col_key,this->masterKey);
        
        printf("Uploading to servers...\n");
        for(int s = 0 ; s < NUM_SERVERS ; s++)
        {
            this->get_data_param[s].column_indexes = selectedIdx;
            this->get_data_param[s].data = this->I_prime;
...
void *Client_ODSE::thread_precomputeAesKey_func(void* param)
{
    ...
    {
        int j = 0;
        for(std::set<TYPE_INDEX>::iterator i = opt->indexes.begin(); i != opt->indexes.end(); ++i,j++)
        {
               odse_keygen->precomputeAES_CTR_keys(&opt->aes_keys[j * MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE/BYTE_SIZE],*i ,opt->dim,opt->isIncremental,opt->block_counter_arr,opt->row_keys,opt->masterKey);
        }
    }
    else
    {
        odse_keygen->precomputeAES_CTR_keys(opt->aes_keys,opt->idx,opt->dim,opt->isIncremental,opt->block_counter_arr,opt->row_keys,opt->masterKey);
    }
    delete odse_keygen;
