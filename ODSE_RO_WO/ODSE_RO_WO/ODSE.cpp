                                                    ...
                                                    MasterKey *pKey)
{
    ...
    for(int i = 0 ; i < n ; i++)
    {
        ...
        //only consider Encrypt block size = 1;
        ...
        for(TYPE_INDEX m =0 ; m < MATRIX_ROW_SIZE; m++)
        {
            for(TYPE_INDEX n = (FF_SIZE/BYTE_SIZE)-1 ; n < MATRIX_PIECE_COL_SIZE; n+=(FF_SIZE/BYTE_SIZE))
            {
                for(int j = BYTE_SIZE-1 ; j >= BYTE_SIZE - (FF_SIZE-(NP_BITS-1)); j--)
                {
                    BIT_CLEAR(&I[m][n].byte_data,j);
                }
 ...
 * @return	0 if successful
 */
int ODSE::saveEncrypted_matrix_to_files(zz_p** I)
{
    int n; 
...
int ODSE::loadEncrypted_matrix_from_files(zz_p** I)
{
    int n; 
    TYPE_INDEX col, row, I_col_idx;
    Miscellaneous misc;
 ...
 * @return	0 if successful
 */
int ODSE::update(unsigned char* I_prime,              
            TYPE_INDEX block_idx,
            zz_p** I)
{
    TYPE_INDEX row, col;
    ...
    int ret;
    unsigned char* tmp = new unsigned char[FF_SIZE/BYTE_SIZE];
    try
    {
        if(ENCRYPT_BLOCK_SIZE>=BYTE_SIZE)
        {
            if(ENCRYPT_BLOCK_SIZE>FF_SIZE)
            {
                begin = block_idx*ENCRYPT_BLOCK_SIZE/FF_SIZE;
                end = begin+ (ENCRYPT_BLOCK_SIZE/FF_SIZE);
                idx = 0;
                for (row=0;row<MATRIX_ROW_SIZE; row++)
                {
                    for(col = begin; col < end; col++,idx+=FF_SIZE/BYTE_SIZE)
                    {
                        memcpy(&I[col][row],&I_prime[idx],FF_SIZE/BYTE_SIZE);
                    }
                }
            }
            else
            {   
                col = block_idx / (FF_SIZE/ENCRYPT_BLOCK_SIZE);
                begin = (block_idx % (FF_SIZE/ENCRYPT_BLOCK_SIZE))*(ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
                for(row = 0 ; row <MATRIX_ROW_SIZE; row++)
                {
                    memcpy(tmp,&I[col][row],FF_SIZE/BYTE_SIZE);
                    memcpy(&tmp[begin], &I_prime[I_prime_idx],ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
                    I_prime_idx +=(ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
                    
                    memcpy(&I[col][row],tmp,FF_SIZE/BYTE_SIZE);
                }
            }
        }
        else 
        {
            col = block_idx / (FF_SIZE/ENCRYPT_BLOCK_SIZE);
            int byte_idx = ((block_idx * ENCRYPT_BLOCK_SIZE / BYTE_SIZE) %  (FF_SIZE/BYTE_SIZE));
                
            begin = (block_idx*ENCRYPT_BLOCK_SIZE) % BYTE_SIZE;
            end = begin + ENCRYPT_BLOCK_SIZE;
            int I_bit_idx, I_prime_bit_idx;
            ...
            for(row = 0 ; row <MATRIX_ROW_SIZE; row++)
            {
                ...
                for( I_bit_idx = begin; I_bit_idx < end; I_bit_idx++,I_prime_idx++)
                {
                    ...
                    if(BIT_CHECK(&I_prime[I_prime_col],I_prime_bit_idx))
                        BIT_SET(&tmp[byte_idx],I_bit_idx);
                    else
                        BIT_CLEAR(&tmp[byte_idx],I_bit_idx);
                }
                memcpy(&I[col][row],tmp,FF_SIZE/BYTE_SIZE);
    ...
    {
        cout << "Error occured in update function " << e.what() << endl;
        ret = -1;
        goto exit;
    }
exit:
...
{    
    ...
    int byte_idx ;
    int ret;
    unsigned char tmp[FF_SIZE/BYTE_SIZE];
    TYPE_INDEX I_prime_idx = 0;
    TYPE_INDEX begin;
    TYPE_INDEX end;
    try
    {
            ...
            if(ENCRYPT_BLOCK_SIZE >= BYTE_SIZE)
            {
                if(ENCRYPT_BLOCK_SIZE>FF_SIZE)
                {
                    begin = index * (ENCRYPT_BLOCK_SIZE/FF_SIZE);
                    end = begin + (ENCRYPT_BLOCK_SIZE/FF_SIZE);
                    
                    for(row = 0 ; row <MATRIX_ROW_SIZE; row++)
                    {
                        for(col = begin; col < end;col++,I_prime_idx+=FF_SIZE/BYTE_SIZE)
                        {
                            memcpy(tmp,&I[col][row],FF_SIZE/BYTE_SIZE);
                            memcpy(&I_prime[I_prime_idx],tmp,FF_SIZE/BYTE_SIZE);
                        }
                    }
                }
                else
                {
                    col = index / (FF_SIZE/ENCRYPT_BLOCK_SIZE);
                    begin = (index % (FF_SIZE/ENCRYPT_BLOCK_SIZE))*(ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
                    for(row = 0 ; row <MATRIX_ROW_SIZE; row++)
                    {
                        memcpy(tmp,&I[col][row],FF_SIZE/BYTE_SIZE);
                        memcpy(&I_prime[I_prime_idx],&tmp[begin],ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
                        I_prime_idx+= (ENCRYPT_BLOCK_SIZE/BYTE_SIZE);
                    }
                }
            }
            ...
            {
                col = index / (FF_SIZE/ENCRYPT_BLOCK_SIZE);
                byte_idx = ((index * ENCRYPT_BLOCK_SIZE / BYTE_SIZE) %  (FF_SIZE/BYTE_SIZE));
                ...
                for(row = 0 ; row <MATRIX_ROW_SIZE; row++)
                {
                    ...
                    for( I_bit_idx = begin; I_bit_idx < end; I_bit_idx++,I_prime_idx++)
                    {
                        ...
                        if(BIT_CHECK(&tmp[byte_idx],I_bit_idx))
                        ...
                        MasterKey* pKey)
{
    ...
    {   
        ...
        //clear positions that must not be touched due to IND-CPA encryption
        TYPE_INDEX kk = 0;
        if(ENCRYPT_BLOCK_SIZE >= BYTE_SIZE)
        {
            for ( std::set<TYPE_INDEX>::iterator i = indexes.begin();  i != indexes.end() ; ++i, kk++)
            {
                int iii = kk*MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE/BYTE_SIZE;
                for(int row = 0 ; row < MATRIX_ROW_SIZE ; row++)
                {
                    int count  = 0;
                    for(TYPE_INDEX j = ((*i)*ENCRYPT_BLOCK_SIZE); j < ((*i) + 1 ) * ENCRYPT_BLOCK_SIZE ; j ++)
                    {
                        
                        if ((j % FF_SIZE) >=  (NP_BITS-1)) 
                        {
                            int bit_number = (j / BYTE_SIZE) %(ENCRYPT_BLOCK_SIZE / BYTE_SIZE);
                            int bit_pos = j %BYTE_SIZE;
                            BIT_CLEAR(&I_prime[iii  + row*ENCRYPT_BLOCK_SIZE/BYTE_SIZE + bit_number].byte_data,bit_pos);
                        }
                    }
                }
            }
        ...
        else
        {
            for ( std::set<TYPE_INDEX>::iterator i = indexes.begin();  i != indexes.end() ; ++i, kk++)
            {
                ...
                for(TYPE_INDEX j = ((*i)*ENCRYPT_BLOCK_SIZE); j < ((*i) + 1 ) * ENCRYPT_BLOCK_SIZE ; j ++)
                {
                    if ((j % FF_SIZE) >=  (NP_BITS-1)) 
                    {
                        int first_pos = j % ENCRYPT_BLOCK_SIZE;
                        for(int row = 0 ; row < MATRIX_ROW_SIZE ; row++,first_pos+=ENCRYPT_BLOCK_SIZE)
                        {
                            int bit_number = first_pos / BYTE_SIZE;
                            int bit_pos = first_pos % BYTE_SIZE;
                            BIT_CLEAR(&I_prime[iii + bit_number].byte_data,bit_pos);    
                        }
...
zz_p ODSE::simpleRecover(zz_p* shares)
{
    zz_p res;
    res = 0;
