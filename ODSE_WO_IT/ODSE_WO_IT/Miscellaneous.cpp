...
int Miscellaneous::write_list_to_file(string filename, string path, std::set<TYPE_INDEX> lst)
{
     std::ofstream output;
     output.open(path+filename);
     TYPE_INDEX i;
     if(lst.size()==0)
         output<<ULONG_MAX;
     for (std::set<TYPE_INDEX>::iterator i = lst.begin() ; i != lst.end();++i)
     {
         output<<(*i) << " ";
     }
     output.close();
...
int Miscellaneous::read_list_from_file(string filename, string path, std::set<TYPE_INDEX> &lst)
{
    ifstream input;
    input.open(path+filename);
    TYPE_INDEX in = ULONG_MAX;
...
int Miscellaneous::write_stash_to_file(string filename, string path, STASH S)
{
    ...
    {
        output<<S[i].ID <<endl;
        output.write((char*)S[i].column_data,MATRIX_ROW_SIZE/BYTE_SIZE);
        output<<endl;
    }
    output.close();
    return 0;
...
int Miscellaneous::read_stash_from_file(string filename, string path, STASH &S)
{
    ...
    {
        string column_data;
        std::getline(input,column_data);
        STASH_DATA s(ID,(unsigned char*) column_data.c_str());
        S.push_back(s);
    }
    input.close();
    return 0;
...
int Miscellaneous::read_matrix_from_file(string filename, string path, zz_p** I, TYPE_INDEX num_row, TYPE_INDEX num_col)
{
    TYPE_INDEX row, col;
    ...
    for(row = 0 ; row < num_row;row++)
    {
        for(col = 0 ; col < num_col; col ++)
        {
            input.read((char*)tmp,FF_SIZE/BYTE_SIZE);
            memcpy(&I[row][col], tmp,FF_SIZE/BYTE_SIZE);
        }
    }
    input.close();
    return 0;
}
int Miscellaneous::write_matrix_to_file(string filename, string path, zz_p** I, TYPE_INDEX num_row, TYPE_INDEX num_col)
{
    TYPE_INDEX row, col;
    
    ofstream input;
    input.open(path+filename, ios::binary | ios::out);
    ...
    for(row = 0 ; row < num_row;row++)
    {
        for(col = 0 ; col < num_col; col ++)
        {
            memcpy(tmp, &I[row][col], FF_SIZE/BYTE_SIZE);
            input.write((char*)tmp,FF_SIZE/BYTE_SIZE);            
        }
    }
    input.close();
    return 0;
