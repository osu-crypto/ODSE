...
Server_ODSE::Server_ODSE(int numThreads)
{
    ...
    search_index_vector = new zz_p[MATRIX_ROW_SIZE];
    this->I = new zz_p *[NUM_ELEMENT_BLOCKS];
    ...
    serializedSearch_vector = new unsigned char[MATRIX_ROW_SIZE*FF_SIZE/BYTE_SIZE];
...
start = time_now;
    ...
    dsse->deserializeFFElementVector(serializedSearch_vector,search_index_vector,MATRIX_ROW_SIZE);
    
    // Multi-thread here
    int step = ceil((double)NUM_ELEMENT_BLOCKS /(double)numThreads);
    int endIdx;
    printf("PIR computation....");
    for(int i = 0, startIdx = 0; i < numThreads , startIdx < NUM_ELEMENT_BLOCKS; i ++, startIdx+=step)
    {
        if(startIdx+step > NUM_ELEMENT_BLOCKS)
            endIdx = NUM_ELEMENT_BLOCKS;
        else
...
void *Server_ODSE::thread_computePIR_func(void* args)
{
    ...
    dsse.search(opt->select_vector,opt->start,opt->end,opt->input,opt->output);
...
int Server_ODSE::updateColumnBlock(zmq::socket_t& socket)
{
    ...
    {
        dsse->update(&serialized_buffer[i*(MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE)/BYTE_SIZE],indexes[i],this->I);
    }
    end = time_now;
    cout<<std::chrono::duration_cast<std::chrono::nanoseconds>(end-start).count()<<" ns"<<endl;
...
auto end = time_now;
    ...
    for(int i = 0 ; i < WRITE_ORAM_LAMBDA ; i++)
    {
        dsse->getBlock(indexes[i],this->I,&serialized_buffer[i*(MATRIX_ROW_SIZE*ENCRYPT_BLOCK_SIZE)/BYTE_SIZE]);
    }
    end = time_now;
